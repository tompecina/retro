...
public class XOR {
   ...
   */
  public XOR(final String name, final int numberInPins) {
    assert name != null;
    assert numberInPins > 0;
    log = Logger.getLogger(getClass().getName() + "." + name);
    log.fine("New XOR creation started: " + name);
    this.name = name;
    ...
    outPin = new OutPin();
    log.fine("New XOR creation completed: " + name);
  }
  ...
  private class InPin extends IOPin {
    ...
    private InPin(final int number) {
      ...
      this.number = number;
      log.finer("New XOR input pin created");
    }
	...
	log.finest("Input " + number + " of XOR '" + name + "' notified");
      }
      outPin.query();
    }
  }
  ...
  private class OutPin extends IOPin {
    ...
    private OutPin() {
      super();
      log.finer("New XOR output pin created");
    }
	...
	if (log.isLoggable(Level.FINER)) {
	  log.finer("Changed output of XOR '" + name + "': " + output);
	}
      }
      return output;
    }
